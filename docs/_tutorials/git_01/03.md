...
for hook_file in ${HOOKS_PATH}/*; do
    ...
    if [ ! -x "${hook_file}" ]; then
        chmod +x "${hook_file}"
        echo "* Le hook '$(basename "${hook_file}")' est maintenant exécutable."
    fi
done
```
...
1. Récupérez le fichier : [actions-quality-git-01]({{ resource_script_actions_quality_git | relative_url }}).
